ALN_CFG = {
    ...
    "temp_dir": config["asm_to_ref"].get(
        "temp_dir", join(OUTPUT_DIR, "asm_to_ref", "temp")
    ),
    "output_dir": config["asm_to_ref"].get(
        "output_dir", join(OUTPUT_DIR, "asm_to_ref")
    ),
    "logs_dir": config["asm_to_ref"].get("log_dir", join(LOG_DIR, "asm_to_ref")),
    "benchmarks_dir": config["asm_to_ref"].get(
        "bmk_dir", join(BENCHMARK_DIR, "asm_to_ref")
    ),
